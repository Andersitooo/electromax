...
-- =========================================================
-- EMPRESA CONFIG ADMIN
-- =========================================================

-- =====================================================================
-- ELECTROMAX - PANEL ADMIN PARA DATOS DE EMPRESA / FACTURACION
-- Ejecutar sobre electro2. Es idempotente, no borra datos.
-- =====================================================================
...
INSERT INTO empresa_config (id, razon_social, nombre_comercial, ruc, direccion_matriz, telefono, email, logo_url, logo_pdf_url, ambiente, establecimiento, punto_emision, moneda, regimen)
VALUES (1, 'ELECTROMAX S.A.S.', 'ElectroMax', '0999999999001', 'Babahoyo, Los Ríos, Ecuador', '04-273-0000', 'facturacion@electromax.com', 'assets/electromax_logo.png', 'assets/electromax_logo_pdf.jpg', 'PRODUCCION', '001', '001', 'USD', 'Documento generado electrónicamente por ElectroMax.')
ON CONFLICT (id) DO UPDATE SET
    logo_pdf_url = COALESCE(empresa_config.logo_pdf_url, EXCLUDED.logo_pdf_url),
    establecimiento = COALESCE(empresa_config.establecimiento, EXCLUDED.establecimiento),
    punto_emision = COALESCE(empresa_config.punto_emision, EXCLUDED.punto_emision),
    moneda = COALESCE(empresa_config.moneda, EXCLUDED.moneda),
    regimen = COALESCE(empresa_config.regimen, EXCLUDED.regimen),
    updated_at = NOW();

-- Verificacion rapida
SELECT id, razon_social, nombre_comercial, ruc, establecimiento, punto_emision, ambiente, logo_url
FROM empresa_config
WHERE id = 1;
